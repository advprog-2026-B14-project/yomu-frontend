...
export const AdminModule = () => {
    ...
    return (
        <div className={`${shell} p-4 md:p-6 lg:p-8`}>
            <div className="max-w-7xl mx-auto space-y-6">
                ...
                <div className={`${panel} p-6 flex flex-col md:flex-row md:items-center justify-between gap-4`}>
                    ...
                    <div className="flex gap-2">
                        <button onClick={fetchUsers} className={secondary}>🔄 Refresh Data</button>
                        <button onClick={handleLogout} className="rounded-xl border border-rose-200 bg-white px-4 py-2.5 text-sm font-bold text-rose-700 transition hover:-translate-y-0.5 hover:border-rose-300 hover:bg-rose-50">Logout</button>
                    </div>
                </div>
                ...
                <div className={`${panel} p-5 flex items-center justify-between`}>
                    <div>
                        <p className="text-xs font-bold uppercase tracking-[0.18em] text-rose-700 mb-1">Manajemen Liga</p>
                        <p className="font-bold text-slate-900">End Current Season</p>
                        <p className="text-xs text-slate-500">Reset poin klan dan mulai musim baru.</p>
                    </div>
                    <button
                        onClick={async () => {
                            if (!confirm("Apakah Anda yakin ingin mengakhiri season sekarang? Tindakan ini tidak dapat dibatalkan!")) return;
                            setActionLoading(true);
                            try {
                                const token = getToken();
                                const res = await fetch(`${API_BASE_URL}/liga/admin/end-season`, {
                                    method: "POST",
                                    headers: { Authorization: `Bearer ${token ?? ""}` }
                                });
                                if (!res.ok) throw new Error("Gagal mengakhiri season");
                                showToast("Season berhasil diakhiri!");
                            } catch (e: any) {
                                showToast(e.message || "Gagal mengakhiri season", "error");
                            } finally {
                                setActionLoading(false);
                            }
                        }}
                        ...
                        className={danger}
                    >
                        {actionLoading ? "Memproses..." : "Akhiri Season 🏁"}
                    </button>
                </div>
                ...
                <div className={`${panel} overflow-hidden`}>
                    <div className="flex flex-col sm:flex-row gap-3 p-5 border-b border-slate-100">
                        <div className="relative flex-1">
                            ...
                            <input type="text" placeholder="Cari nama, email, atau username..." value={search} onChange={(e) => setSearch(e.target.value)} className={`${inputStyle} pl-9`} />
                        </div>
                        <select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)} className={`${inputStyle} sm:w-48 cursor-pointer`}>
                    ...
                    {loading ? (
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                                ...
                                {paginated.length === 0 ? (
                                    <tr>
                                        <td colSpan={5} className="py-12 text-center text-slate-400 font-medium">Tidak ada pengguna ditemukan</td>
                                    </tr>
                                ) : (
                                    paginated.map((u) => (
                                        <tr key={u.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                                            ...
                                            <td className="px-5 py-4">
                                                    <span className={`inline-block px-2.5 py-1 rounded-full text-[11px] font-bold tracking-wide uppercase ${u.role === "ADMIN" ? "bg-emerald-100 text-emerald-800" : "bg-slate-100 text-slate-600"}`}>
                                                        {u.role}
                                                    </span>
                                            </td>
                                            ...
                                            <td className="px-5 py-4">
                                                <div className="flex gap-2">
                                                    <button onClick={() => { setRoleModal({ open: true, user: u }); setNewRole(u.role); }} className="flex items-center gap-1 px-3 py-1.5 text-xs font-bold border border-slate-200 text-slate-700 rounded-lg hover:bg-slate-100 transition-colors">🛡️ Role</button>
                                                    <button onClick={() => setDeleteModal({ open: true, user: u })} className="flex items-center gap-1 px-3 py-1.5 text-xs font-bold bg-rose-50 border border-rose-200 text-rose-700 rounded-lg hover:bg-rose-100 transition-colors">🗑️ Hapus</button>
                                                </div>
                                            </td>
                                        </tr>
                                    ))
                            ...
                            {totalPages > 1 && (
                                <div className="flex items-center justify-between px-5 py-4 border-t border-slate-100 bg-slate-50/30">
                                    <span className="text-xs font-medium text-slate-500">Menampilkan {(page - 1) * PER_PAGE + 1}–{Math.min(page * PER_PAGE, filtered.length)} dari {filtered.length}</span>
                                    <div className="flex gap-1.5">
                                        {Array.from({ length: totalPages }, (_, i) => i + 1).map((n) => (
                                            <button key={n} onClick={() => setPage(n)} className={`w-8 h-8 flex items-center justify-center text-xs font-bold rounded-lg transition-colors ${n === page ? "bg-emerald-700 text-white shadow-sm shadow-emerald-700/20" : "bg-white border border-slate-200 text-slate-600 hover:bg-slate-50"}`}>
                                                {n}
                                            </button>
                                        ))}
                                    </div>
            ...
            {roleModal.open && (
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className={`${panel} w-full max-w-sm p-6`}>
                        ...
                        <select value={newRole} onChange={(e) => setNewRole(e.target.value)} className={`${inputStyle} mb-6`}>
                            <option value="USER">User (Learner)</option>
                            <option value="ADMIN">Admin</option>
                        </select>
                        <div className="flex gap-3 justify-end">
                            <button onClick={() => setRoleModal({ open: false, user: null })} className="px-4 py-2.5 text-sm font-bold border border-slate-200 text-slate-700 rounded-xl hover:bg-slate-50 transition">Batal</button>
                            <button onClick={handleRoleConfirm} disabled={actionLoading} className={primary}>{actionLoading ? "Menyimpan..." : "Simpan Perubahan"}</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {deleteModal.open && (
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className={`${panel} w-full max-w-sm p-6`}>
                        ...
                        <p className="text-sm font-medium text-slate-600 mb-6 leading-relaxed">Apakah Anda yakin ingin menghapus <strong className="text-slate-900">{deleteModal.user?.fullName}</strong>? Tindakan ini permanen.</p>
                        <div className="flex gap-3 justify-end">
                            <button onClick={() => setDeleteModal({ open: false, user: null })} className="px-4 py-2.5 text-sm font-bold border border-slate-200 text-slate-700 rounded-xl hover:bg-slate-50 transition">Batal</button>
                            <button onClick={handleDeleteConfirm} disabled={actionLoading} className={danger}>{actionLoading ? "Menghapus..." : "Ya, Hapus"}</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {toast && (
                <div className={`fixed bottom-6 right-6 px-4 py-3 rounded-xl border text-sm font-bold shadow-lg z-50 transition-all ${toast.type === "success" ? "bg-emerald-50 border-emerald-200 text-emerald-800" : "bg-rose-50 border-rose-200 text-rose-800"}`}>
                    {toast.msg}
                </div>
            )}
        </div>
